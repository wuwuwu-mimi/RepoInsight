...
from repoinsight.models.analysis_model import AnalysisRunResult, ApiRouteSummary, ClassSummary, FunctionSummary, KeyFileContent, ModuleRelation, ProjectProfile
...
def test_investigate_code_hits_extracts_route_symbol_and_calls() -> None:
    ...
    try:
        ...
        context_lines = build_code_investigation_context_lines(investigation, max_lines=8)
        assert context_lines[0].startswith('[code_agent] 已定位到接口 /login 的实现入口')
        assert any('app.py:L18' in line for line in context_lines)
        assert any('源码片段' in line for line in context_lines)
    finally:
        clear_code_agent_cache()
...
def test_investigate_code_hits_can_follow_cross_file_call_chain() -> None:
    ...
    try:
        ...
        assert any(step.label == 'auth_service.login_user' and step.relation_type == 'delegate_service' for step in investigation.trace_steps)
        assert any(step.label == 'session_repo.persist_session' and step.relation_type == 'delegate_repository' for step in investigation.trace_steps)
        ...
        assert any(
            ...
            for item in investigation.relation_chain_details
        )
        assert investigation.relevance_score >= 0.4
        assert investigation.confidence_level in {'medium', 'high'}
        ...
        assert any('关系链类型：POST /session -[delegate_service]-> auth_service.login_user -[delegate_repository]-> session_repo.persist_session' in line for line in context_lines)
        assert any('[relation=delegate_service]' in line for line in context_lines)
        assert any('[relation=delegate_repository]' in line for line in context_lines)
...
def test_investigate_code_hits_can_follow_module_relations_from_key_file_summary() -> None:
    temp_dir = Path('data/test_code_agent_module_relations')
    clone_root = Path('data/test_code_agent_module_relations_repo')
    shutil.rmtree(temp_dir, ignore_errors=True)
    shutil.rmtree(clone_root, ignore_errors=True)
    original_get_clone_path = code_agent_module.get_clone_path
    try:
        ...
        real_clone_path = _build_cross_file_clone_repo(clone_root)
        code_agent_module.get_clone_path = lambda repo_id: real_clone_path
        result = _build_cross_file_analysis_result()
        result.key_file_contents = [
            KeyFileContent(
                path='api/routes.py',
                size_bytes=128,
                content='from services import auth_service\n\ndef create_session(payload):\n    return auth_service.login_user(payload)\n',
            ),
            KeyFileContent(
                path='services/auth_service.py',
                size_bytes=128,
                content='from repositories import session_repo\n\ndef login_user(payload):\n    return session_repo.persist_session(payload[\"user_id\"])\n',
            ),
        ]
        result.project_profile.module_relations = [
            ...
            ModuleRelation(
                source_path='services/auth_service.py',
                target='repositories.session_repo',
                relation_type='import',
                line_number=1,
            ),
        ]
        ...
        save_repo_documents('demo/sample', documents, target_dir=str(temp_dir))
        key_file_document = next(
            item for item in documents
            if item.doc_type == 'key_file_summary' and item.source_path == 'api/routes.py'
        )

        investigation = investigate_code_hits(
            '认证模块依赖关系是怎样的？',
            [SearchHit(document=key_file_document, score=1.0, snippet='api/routes.py 依赖 services.auth_service')],
            'architecture',
            repo_id='demo/sample',
            target_dir=str(temp_dir),
            max_follow_depth=2,
        ...
        assert investigation is not None
        assert any(step.label == 'api/routes.py' for step in investigation.trace_steps)
        assert any(step.relation_type == 'import_module' for step in investigation.trace_steps)
        context_lines = build_code_investigation_context_lines(investigation)
        assert any(
            (
                '关系链类型：api/routes.py -[import_module]-> create_session -[call]-> auth_service.login_user' in line
                or '关系链类型：api/routes.py -[import_module]-> auth_service.login_user -[delegate_repository]-> session_repo.persist_session' in line
            )
            for line in context_lines
        )
        assert 'services/auth_service.py' in investigation.source_paths
    finally:
        clear_code_agent_cache()
        code_agent_module.get_clone_path = original_get_clone_path
        shutil.rmtree(temp_dir, ignore_errors=True)
...
def test_run_multi_agent_answer_runs_code_agent_for_implementation_questions() -> None:
    ...
    try:
        ...
        assert trace_by_role['code_agent'].structured_output is not None
        ...
        assert trace_by_role['revision_agent'].structured_output is not None
        assert trace_by_role['code_agent'].structured_output is not None
        assert trace_by_role['code_agent'].structured_output.conclusions
        assert trace_by_role['verifier_agent'].structured_output.metadata['verdict'] in {'passed', 'warning', 'failed'}
        assert coordinated.verification_result is not None
        assert '实现链路：' in coordinated.answer_result.answer
        assert '实现过程：' in coordinated.answer_result.answer
...
def test_run_multi_agent_answer_runs_architecture_agent_for_architecture_questions() -> None:
    temp_dir = Path('data/test_code_agent_architecture_coordinator')
    clone_root = Path('data/test_code_agent_architecture_repo')
    shutil.rmtree(temp_dir, ignore_errors=True)
    shutil.rmtree(clone_root, ignore_errors=True)
    original_get_clone_path = code_agent_module.get_clone_path
    try:
        ...
        assert coordinated.code_investigation is not None
        assert coordinated.shared_context['code_agent_enabled'] is False
        ...
        assert '模块链路：' in coordinated.answer_result.answer
        assert '架构线索' in coordinated.answer_result.answer
        assert 'auth_service.login_user' in coordinated.answer_result.answer
        assert 'session_repo.persist_session' in coordinated.answer_result.answer
    finally:
        clear_code_agent_cache()
        code_agent_module.get_clone_path = original_get_clone_path
        shutil.rmtree(temp_dir, ignore_errors=True)
...
def test_investigate_code_hits_prefers_embedded_chunk_snippet_for_implementation() -> None:
    temp_dir = Path('data/test_code_agent_chunk_snippet')
    clone_root = Path('data/test_code_agent_chunk_snippet_repo')
    shutil.rmtree(temp_dir, ignore_errors=True)
    shutil.rmtree(clone_root, ignore_errors=True)
    original_get_clone_path = code_agent_module.get_clone_path
    try:
        ...
        assert investigation is not None
        assert any(
            step.label == 'AuthService.handle_login' and step.snippet and 'def handle_login' in step.snippet
            for step in investigation.trace_steps
        )
        context_lines = build_code_investigation_context_lines(investigation, max_lines=8)
        assert any('源码片段 AuthService.handle_login：' in line for line in context_lines)
        assert any('def handle_login' in line for line in context_lines)
    finally:
        clear_code_agent_cache()
        code_agent_module.get_clone_path = original_get_clone_path
        shutil.rmtree(temp_dir, ignore_errors=True)
